  ...
  return (
    ...
    >
      <button className="text-sm font-medium text-white/70 group-hover:text-white transition-colors relative flex items-center gap-1">
        Categories
        <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${isOpen ? 'rotate-180 text-cta-primary' : ''}`} />
          ...
          >
            ...
            <div className="w-2/3 p-8 bg-white/5 border-r border-white/10">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/store" className="text-cta-primary text-sm flex items-center gap-1 hover:text-white transition-colors">
                  View All <ArrowRight className="w-3 h-3" />
                </Link>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {CATEGORIES.map((cat) => (
                  <Link href={`/store?category=${encodeURIComponent(cat.name)}`} key={cat.name} className="group/item relative rounded-xl overflow-hidden aspect-[4/3] flex items-end p-4 border border-white/10 hover:border-cta-primary/50 transition-all">
                    ...
                    <span className="relative z-10 text-white font-bold tracking-wide group-hover/item:text-cta-primary transition-colors text-sm">{cat.name}</span>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="w-1/3 p-8 flex flex-col justify-between">
              <div>
                <h3 className="text-slate-300 font-bold text-xs uppercase tracking-widest mb-6">Explore More</h3>
                <div className="flex flex-col gap-4">
                  {SECONDARY_CATEGORIES.map((cat) => (
                    <Link key={cat} href={`/store?category=${encodeURIComponent(cat)}`} className="text-white/70 text-sm hover:text-white hover:translate-x-1 transition-all flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-cta-primary/50" />
                      {cat}
                    </Link>
                  ))}
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/10">
                <Link href="/store?trending=true" className="flex items-center gap-3 group/trend mb-4">
                  <div className="w-10 h-10 rounded-full bg-amber-500/10 flex items-center justify-center border border-amber-500/20 group-hover/trend:bg-amber-500/20 transition-colors">
                    <span className="text-amber-500 text-lg leading-none">🔥</span>
                  </div>
                  <div>
                    <p className="text-white text-sm font-bold">Trending Now</p>
                    <p className="text-slate-300 text-xs group-hover/trend:text-white transition-colors">See what's popular</p>
                  </div>
                </Link>

                <h3 className="text-slate-300 font-bold text-xs uppercase tracking-widest mb-3 mt-4">Portals</h3>
