      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
        <div>
          <h1 className="text-4xl font-bold text-white mb-2 flex items-center gap-3">
            <Gavel className="w-8 h-8 text-cta-primary" />
            Nova Sphere Auctions
          </h1>
          <p className="text-slate-300 text-lg">Bid on exclusive, limited-edition, and rare items.</p>
        </div>
      </div>

      {auctions.length === 0 ? (
        <div className="glass-panel p-12 text-center border-dashed border-2 border-white/20">
          <Gavel className="w-12 h-12 text-slate-300 mx-auto mb-4 opacity-50" />
          <h3 className="text-2xl font-bold text-white mb-2">No Active Auctions</h3>
          <p className="text-slate-300">Check back later for new premium items.</p>
        </div>
      ) : (
                ...
                <div className="glass-panel glass-panel-glow h-full flex flex-col overflow-hidden transition-all duration-300 transform group-hover:-translate-y-1">
                  ...
                  <div className="relative aspect-square bg-black/40 p-6 flex items-center justify-center">
                    {primaryImage ? (
                      ...
                      <div className="w-full h-full bg-slate-500/20 rounded-xl" />
                    )}
                    
                    {/* Badges */}
                    <div className="absolute top-4 left-4 flex flex-col gap-2">
                      <span className="bg-cta-primary text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                        AUCTION
                      </span>
                      {isEndingSoon && (
                        <span className="bg-amber-500 text-black text-xs font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <h3 className="text-lg font-bold text-white mb-2 line-clamp-2 group-hover:text-cta-primary transition-colors">
                      {auction.product.name}
                    </h3>
                    
                    <div className="mt-auto pt-4 flex items-end justify-between border-t border-white/10">
                      <div>
                        <p className="text-xs text-slate-300 mb-1">Current Bid</p>
                        <p className="text-xl font-bold text-white">
                          ${auction.currentBid.toNumber() > 0 ? auction.currentBid.toFixed(2) : auction.baseAmount.toFixed(2)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-slate-300 mb-1">{auction._count.bids} Bids</p>
