...
export default async function AdminCustomersPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-slate-300 mb-8">View and manage customer accounts.</p>

      <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-white/5 border-b border-white/10">
            <tr>
              <th className="p-4 text-slate-300 font-medium">Customer ID</th>
              <th className="p-4 text-slate-300 font-medium">Name</th>
              <th className="p-4 text-slate-300 font-medium">Email</th>
              <th className="p-4 text-slate-300 font-medium">Total Orders</th>
          ...
          <tbody className="divide-y divide-white/5">
            {users.length === 0 ? (
              <tr>
                <td colSpan={5} className="p-8 text-center text-slate-300">No customers found.</td>
              </tr>
            ) : (
              users.map((user) => (
