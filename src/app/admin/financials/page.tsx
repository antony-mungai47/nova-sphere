    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-300">Real-time revenue, taxes, and transaction history.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
        <div className="glass-panel p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-[40px] -mr-10 -mt-10 group-hover:bg-emerald-500/20 transition-all" />
          ...
          <h3 className="text-slate-300 mb-1 text-sm relative z-10">Gross Revenue</h3>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          ...
          <h3 className="text-slate-300 mb-1 text-sm relative z-10">Total Refunded</h3>
          <p className="text-2xl font-bold text-white relative z-10">${totalRefunded.toFixed(2)}</p>
        </div>

        <div className="glass-panel p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-cta-primary/10 rounded-full blur-[40px] -mr-10 -mt-10 group-hover:bg-cta-primary/20 transition-all" />
          <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 mb-4 relative z-10">
            <DollarSign className="w-5 h-5 text-cta-primary" />
          </div>
          <h3 className="text-slate-300 mb-1 text-sm relative z-10">Net Revenue</h3>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          ...
          <h3 className="text-slate-300 mb-1 text-sm relative z-10">Taxes Collected</h3>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          ...
          <h3 className="text-slate-300 mb-1 text-sm relative z-10">Shipping Collected</h3>
          ...
          <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden">
            <table className="w-full text-left">
              <thead className="bg-white/5 border-b border-white/10">
                <tr>
                  <th className="p-4 text-slate-300 text-xs font-medium uppercase tracking-wider">Date</th>
                  <th className="p-4 text-slate-300 text-xs font-medium uppercase tracking-wider">Type</th>
                  <th className="p-4 text-slate-300 text-xs font-medium uppercase tracking-wider">Gateway ID</th>
                  <th className="p-4 text-slate-300 text-xs font-medium uppercase tracking-wider">Status</th>
                  <th className="p-4 text-slate-300 text-xs font-medium uppercase tracking-wider text-right">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-300">No transactions recorded yet.</td>
                  </tr>
                ) : (
                  transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-white/5 transition-colors">
                      <td className="p-4 text-slate-300 text-sm">{new Date(tx.createdAt).toLocaleDateString()}</td>
                      <td className="p-4">
                        <span className={`px-2 py-1 text-xs rounded-full border ${tx.type === 'CHARGE' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                          {tx.type}
                        </span>
                      </td>
                      <td className="p-4 text-slate-300 font-mono text-xs">{tx.providerId || "SIMULATED"}</td>
                      <td className="p-4">
                        <span className="text-slate-300 text-sm">{tx.status}</span>
                      </td>
        ...
        <div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4">
            {recentRefunds.length === 0 ? (
              <p className="text-slate-300 text-sm">No refunds processed recently.</p>
            ) : (
              recentRefunds.map(refund => (
                <div key={refund.id} className="flex items-center justify-between border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  <div>
                    ...
                    <p className="text-slate-300 text-xs font-mono">{refund.providerId || "SIMULATED"}</p>
                  </div>
